      ...
      <div className="border-2 border-red-500 bg-white p-4 rounded-lg mb-6">
        <div className={`flex ${showMaintenance ? 'justify-between' : 'justify-around'} items-center`}>
          <div className="text-center">
            <div className="text-xl font-bold">₹ {property.homeProperty?.expectedRent || 'N/A'}</div>
            <div className="text-sm text-gray-500">Rent</div>
          </div>
          <div className="text-center">
            <div className="text-xl font-bold">₹ {property.homeProperty?.expectedDeposit || 'N/A'}</div>
          ...
          {showMaintenance && (
            <div className="text-center">
              <div className="text-xl font-bold">₹ {property.homeProperty?.maintenanceAmount || 'N/A'}</div>
        ...
        <div className="flex flex-col gap-4">
          <div className="grid grid-cols-2 gap-0 border border-gray-300 rounded-lg">
            ...
            <div className="flex flex-row items-center justify-start border p-4">
              <span className="mr-2">👨‍👩‍👧‍👦</span>
              <div>
                <div className="font-bold">{property.homeProperty?.preferredTenants?.join(', ') || 'N/A'}</div>
                <div className="text-sm text-gray-500">Preferred Tenants</div>
              </div>
            </div>
      ...
      <div className="p-4 mt-6">
        ...
        <div className="border-2 border-red-500 bg-white p-4 rounded-lg">
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="border p-2 rounded">
              <strong>Non-Veg Allowed:</strong> {property.homeProperty?.nonVegAllowed ? 'Yes' : 'No'}
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="text-center">
          ...
          <div className="border-2 border-red-500 bg-white p-4 rounded-lg" style={{ width: '500px' }}>
            <strong className="text-muted-foreground mb-6 block">
              You need to pay a token amount, which is 10% of your rent to reserve this property.
            </strong>
            <button
              onClick={() => handleRazorpayPayment(Math.round((property.homeProperty?.expectedRent || 1000) * 0.1 * 100))}
              className="bg-green-500 text-white py-2 px-6 rounded-lg mx-auto block"
            >
              Proceed to Pay ₹{Math.round((property.homeProperty?.expectedRent || 1000) * 0.1)}
            </button>
          </div>
        </div>
      </div>
